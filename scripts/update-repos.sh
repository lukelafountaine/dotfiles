...
source "${SCRIPT_DIR}/../shell/utilities.sh"
      ...
      if [ -d "./${REPO_NAME}" ]; then
         ...
         if [ "${DEV_QUERY}" == "null" ]; then
            ...
            if [ "${FORK_ID}" == "" ]; then
               ...
               git clone "${CLONE_URL}" --origin canonical
               ...
               if [ "${FORK_URL}" == "" ]; then
                  ...
                  silentPushd "./${REPO_NAME}" && git remote add origin "${FORK_URL}" && git fetch --all && silentPopd
               fi
            fi
         fi
      fi
...
processConfigFile() {
   for KEY in $(cat "${CONFIG_FILE}" | jq -r 'keys_unsorted[]'); do
